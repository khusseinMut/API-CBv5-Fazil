...
import {createService, deleteService, getAll, getById, getByName, updateService} from "../helpers/service_helper";
...
describe('Service tests', ()=>{
    describe('Create service', ()=>{
        ...
        before(async()=>{
            ...
            response = await createService(faker.word.noun(), vendorId, faker.datatype.number(100) , faker.datatype.number(100) )
        })
    ...
    describe('Update service', ()=>{
        ...
        before(async()=>{
            vendorId = (await createVendor()).body.payload
            serviceId = (await createService(faker.word.noun(), vendorId, faker.datatype.number(100) , faker.datatype.number(100))).body.payload
            response = await updateService(vendorId, serviceId, faker.datatype.number(100) , faker.datatype.number(100))
        })
        it('Response status code is 200', () => {
            expect(response.statusCode).to.eq(200)
        })
    ...
    describe('Check if the service was actually updated', ()=>{
        ...
        before(async()=>{
            ...
            serviceId = (await createService(faker.word.noun(), vendorIdOne, faker.datatype.number(100) , faker.datatype.number(100))).body.payload
            beforeVendor = (await getById(serviceId)).body.payloa
            vendorIdTwo = (await createVendor()).body.payload
            response = await updateService(vendorIdTwo, serviceId, faker.datatype.number(100) , faker.datatype.number(100))
